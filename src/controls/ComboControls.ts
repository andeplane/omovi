...
function getPinchInfo(domElement: HTMLElement, touches: TouchList) {
  ...
  const offsets = touchList.map(({ clientX, clientY }) =>
    getHTMLOffset(domElement, clientX, clientY)
  )
...
export class ComboControls extends EventDispatcher<CameraUpdateEvent> {
  ...
  private reusableVector2: Vector2 = new Vector2()
  ...
  constructor(
    ...
    super()
    this.camera = camera
    this.reusableCamera = camera.clone() as PerspectiveCamera
  ...
  public update = (deltaTime: number): boolean => {
    ...
    sphericalEnd.theta =
      Math.sign(sphericalEnd.theta) *
      Math.min(Math.abs(sphericalEnd.theta), 2.0 * Math.PI)
    ...
    const deltaFactor = wantDamping
      ? Math.min(dampingFactor * this.targetFPSOverActualFPS, 1)
      : 1
  ...
  private rotate = (deltaX: number, deltaY: number) => {
    ...
    const azimuthAngle =
      (this.firstPersonMode
        ? this.mouseFirstPersonRotationSpeed
        : this.pointerRotationSpeedAzimuth) * deltaX
    const polarAngle =
      (this.firstPersonMode
        ? this.mouseFirstPersonRotationSpeed
        : this.pointerRotationSpeedPolar) * deltaY
  ...
  private handleKeyboard = () => {
    ...
    const horizontalMovement =
      Number(keyboard.isPressed('a')) - Number(keyboard.isPressed('d'))
    const verticalMovement =
      Number(keyboard.isPressed('e')) - Number(keyboard.isPressed('q'))
  ...
  private rotateFirstPersonMode = (
    ...
    targetEnd.addVectors(
      reusableCamera.position,
      reusableVector3.multiplyScalar(distToTarget)
    )
    sphericalEnd.setFromVector3(
      reusableVector3.subVectors(reusableCamera.position, targetEnd)
    )
    sphericalEnd.makeSafe()
  }
  ...
  private dollyPerspectiveCamera = (
    ...
    const distFromCameraToScreenCenter = Math.tan(
      ThreeMath.degToRad(90 - (camera as PerspectiveCamera).fov * 0.5)
    )
    ...
    reusableCamera.position.setFromSpherical(sphericalEnd).add(targetEnd)
    reusableCamera.lookAt(targetEnd)
    this.reusableVector2.set(x, y)
    raycaster.setFromCamera(this.reusableVector2, reusableCamera)
    ...
    if (radius < minDistance) {
      ...
      if (dynamicTarget) {
        ...
        targetEnd.add(
          cameraDirection.normalize().multiplyScalar(Math.abs(deltaDistance))
        )
      } else {
    ...
    const rayDirection = raycaster.ray.direction
      .normalize()
      .multiplyScalar(distFromRayOrigin)
